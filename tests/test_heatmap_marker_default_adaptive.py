...
def test_default_marker_is_adaptive_only():
    geom = {
    ...
    F = np.exp(-((Y + 0.25) ** 2 + (Z - 0.0) ** 2) / (0.3 ** 2))
    fig, _ = plot_geometry_and_heatmap(
        ...
        vf_field=F, vf_grid={"y": gy, "z": gz}, adaptive_peak_yz=(-0.255, 0.0),
        # marker_mode omitted -> defaults to adaptive
        title="t",
    )
    meta = getattr(fig, "_vf_plot_meta", {})
    placed = meta.get("markers", {})
    assert placed.get("adaptive") is not None
    assert placed.get("grid") is None
